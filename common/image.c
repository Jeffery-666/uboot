...
int fit_image_set_hashes(void *fit, int image_noffset)
{
	...
	int noffset;
	int ndepth;
	uint8_t key_schedule[AES_EXPAND_KEY_LENGTH];
	uint8_t passwd[AES_KEY_LENGTH];
	uint8_t passwd0[AES_KEY_LENGTH];
	uint8_t		tmp_value[FIT_MAX_CRYPT_HASH_LEN];
	int i = 0;
	int len = 0;
	...
	for (ndepth = 0, noffset = fdt_next_node(fit, image_noffset, &ndepth);
		...
		if (ndepth == 1) {
			...
			memcpy(passwd, DEFAULT_HASH_PASSWD1, 5);
			memcpy(passwd+5, DEFAULT_HASH_PASSWD3, 5);
			memcpy(passwd+10, DEFAULT_HASH_PASSWD5, 6);

			memset(passwd0, 0, sizeof(passwd0));
			memcpy(passwd0, DEFAULT_HASH_PASSWD0, 5);
			memcpy(passwd0+5, DEFAULT_HASH_PASSWD2, 5);
			memcpy(passwd0+10, DEFAULT_HASH_PASSWD4, 6);

			aes_expand_key((uint8_t *)passwd, (uint8_t *)key_schedule);

			memset(tmp_value, 0 , sizeof(tmp_value));
			for(i = 0; i < (value_len + (AES_KEY_LENGTH) - 1)/(AES_KEY_LENGTH); i++){
				aes_encrypt(value + i*(AES_KEY_LENGTH), (uint8_t *)key_schedule, tmp_value + i*(AES_KEY_LENGTH));	
				len += AES_KEY_LENGTH;
			}

			printf("encrypt Hash value: ");
			for (i = 0; i < len; i++){
				printf("%02x", tmp_value[i]);
			}
			printf("\n");

			if (fit_image_hash_set_value(fit, noffset, tmp_value,
							len)) {
...
int fit_image_check_hashes(const void *fit, int image_noffset)
{
	...
	for (ndepth = 0, noffset = fdt_next_node(fit, image_noffset, &ndepth);
		...
		if (ndepth == 1) {
			...
			memset(tmp_value, 0 , sizeof(tmp_value));
			for(i = 0; i < (fit_value_len + (AES_KEY_LENGTH) - 1)/(AES_KEY_LENGTH); i++){
				aes_decrypt(fit_value + i*(AES_KEY_LENGTH), (uint8_t *)key_schedule, tmp_value + i*(AES_KEY_LENGTH));	
			}
#endif
			if (calculate_hash(data, size, algo, value,
						&value_len)) {
			...
			if (memcmp(value, tmp_value, value_len) != 0) {
				err_msg = " error!\nBad hash value";
				goto error;
			}
#else
